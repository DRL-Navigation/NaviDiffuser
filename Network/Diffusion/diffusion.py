...
class ClassifyFreeDDIM:
    ...
    def sample(self, x, c):
        ...
        for i in reversed(range(0, self.ddim_timesteps)):
            ...
            alpha_cumprod_t = self._extract(self.alphas_cumprod, t, action.shape)
            alpha_cumprod_t_prev = self._extract(self.alphas_cumprod, prev_t, action.shape)
    
            # 2. predict noise using model
            pred_noise_c = self.model.sample(x, t, c, torch.ones(batch_size).bool().to(device=device))
            pred_noise_none = self.model.sample(x, t, c, torch.zeros(batch_size).bool().to(device=device))
            pred_noise = (1+self.guide_w)*pred_noise_c - self.guide_w*pred_noise_none
            
            # 3. get the predicted x_0
            pred_x0 = (action - torch.sqrt((1. - alpha_cumprod_t)) * pred_noise) / torch.sqrt(alpha_cumprod_t)
            ...
            x_prev = torch.sqrt(alpha_cumprod_t_prev) * pred_x0 + pred_dir_xt + sigmas_t * torch.randn_like(action)
    ...
    def learn(self, x, c, info):
        x_ = x['action']
        batch_size, device = x_.shape[0], x_.device
        self.model.train()
        
        noise = torch.randn_like(x_)
        t = torch.randint(0, self.timesteps, (batch_size, 1)).to(device=device, dtype=torch.float32)
        x['action'] = self._q_sample(x_, t, noise)
        c_mask = (torch.rand(batch_size) > self.guide_dropout).to(device)
        predicted_noise = self.model(x, t, c, c_mask)

        batch, up_batch, topk = info['batch'], info['up_batch'], info['topk']
        loss = F.mse_loss(noise, predicted_noise, reduction="none")
        loss1, loss2 = loss.split([batch, up_batch], dim=0)
        loss = (1.0-topk) * torch.mean(loss1) + topk * torch.mean(loss2)
        return loss
