...
class PPO(Basenn):
    ...
    def learn(self, data: Experience):
        ...
        for _ in range(self.training_iter_time):
            ...
            # duel ppo
            actor_loss_pure = -torch.mean(torch.where(data.advs > 0, torch.min(ratio * data.advs,
            ...
            actor_loss = actor_loss_pure - entropy_loss * self.ent_loss_theta
            total_loss = actor_loss + v_loss * self.v_loss_theta
