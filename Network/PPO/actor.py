...
class GaussionActor(Actor):
    def __init__(self,
        ...
        super(GaussionActor, self).__init__(
                                            pre=pre,
                                            device=device)
        self.actor_linear_v = mlp([(last_input_dim, 1, "sigmoid")])
        self.actor_linear_w = mlp([(last_input_dim, 1, "sigmoid")])
        self.actor_linear_std = mlp([(last_input_dim, action_output_dim, "sigmoid")])


    def _distribution(self, x, play_mode=False):

        v_mu = self.actor_linear_v(x)
        w_mu = (self.actor_linear_w(x)-0.5) * 1.8
        mu = torch.cat([v_mu, w_mu], dim=1)
        std = self.actor_linear_std(x)
        if play_mode:
            return mu
        else:
            pi = Normal(mu, std)
