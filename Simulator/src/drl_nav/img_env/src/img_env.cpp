...
void ImgEnv::_init_param_from_py(comn_pkg::InitEnv::Request &req){
    ...
    random_init_speed_ = req.random_init_speed;
...
//        eps_res_msg.robots_res[i].info.size = robots_[i].sizes_;
        ...
        if(random_init_speed_ == true){
            double rand_v = robots_[i].limiter_lin_.min_velocity + (robots_[i].limiter_lin_.max_velocity - robots_[i].limiter_lin_.min_velocity)*double(std::rand()%1000)/1000;
            double rand_w = robots_[i].limiter_ang_.min_velocity + (robots_[i].limiter_ang_.max_velocity - robots_[i].limiter_ang_.min_velocity)*double(std::rand()%1000)/1000;
            robots_[i].last1_vw_ = robots_[i].last0_vw_ = Point2d(rand_v, rand_w);
        }
...
void ImgEnv::show_gui()
{
        ...
        {   
            ...
            {
                tf::Transform tf_point_base;
                tf_point_base.setOrigin(tf::Vector3(robots_[i].trajectory[p].x, robots_[i].trajectory[p].y, 0.0));
                tf::Quaternion qu;
                qu.setRPY(0, 0, 0);
                tf_point_base.setRotation(qu);
                tf::Transform tf_point_world = robots_[i].get_base_world() * tf_point_base;
                tf::Vector3 t = tf_point_world.getOrigin();
                Point2i point;
                EnvMap_maps_.visual_map_.world2map(Point2d(t.x(), t.y()), point);
