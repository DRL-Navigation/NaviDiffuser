...
class GazeboEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        print('reset', rospy.wait_for_message(self.scan_topic, LaserScan, timeout=120), flush=True)
    ...
    def _norm_lasers(self, lasers_) -> np.ndarray:
        ...
        np_lasers = np.where(np_lasers==0, self.laser_max, np_lasers)
        return np_lasers
    ...
    def _is_done(self, state):
        ...
        if self.get_goal_dist(vector_state) < 1:
            print("robot arrive !")
            return 5
        elif self.gazebo_connection.get_hard_collision():
            self.is_collision = 1
