...
class BagReader(object):
    def __init__(self, args):
        ...
        self.topic2 = args.topic.replace('episode', 'reward')

    def reshape_list(self, data, dims):
        if not dims:
            return data.pop(0)
        return [self.reshape_list(data, dims[1:]) for _ in range(dims[0].size)]
    ...
    def distill_img_env_bag(self, interval=1):
        ...
        ep_i = 0
        for topic, msg, t in self.bag.read_messages(topics=[self.topic2]):
            reward_list = np.array(self.reshape_list(list(msg.data), msg.layout.dim))
            for i in range(reward_list.shape[1]):
                summary[ep_i]['robot'][i]['reward'] = reward_list[:, i, :]
            ep_i += 1
    ...
    def _plot_t_ep(self, summary, episode, ax, xy_lim, obs_color, robot_every_circle=True):
        ...
        for i in range(max_step):
            for r in range(robot_total):
                ...
                reward = np.array(summary[episode]['robot'][r]['reward'])
                reward_color = reward[:,:3] / 10.0
                reward_color = np.vstack([reward_color, reward_color[-1]])
                        ...
                        if summary[episode]['robot'][r]['shape'] == 'rectangle':
                            if robot_every_circle:
                                ax.add_artist(plt.Polygon(summary[episode]['robot'][r]['bbx'][i], True, alpha=0.8,
                                                      color=darker(reward_color[i], alpha[i]), ec=None))
                            else:
                                if i % 2 == 0:
                                    ax.add_artist(plt.Polygon(summary[episode]['robot'][r]['bbx'][i], True, alpha=0.8,
                                                      color=darker(reward_color[i], alpha[i]), ec=None))
            ...
            for r in range(ped_total):
                ...
                if i < len(plot_xs):
                    if self.type == 'image' or self.type == 'image_ped':
                        ...
                        if i % 2 == 0 :
                            ax.add_artist(Circle(xy=(plot_xs[i], plot_ys[i]), radius=0.2, alpha=0.6,
                                                 facecolor=darker(mcolors.to_rgb(plot_colors[r + robot_total]),
                                                                  alpha[i]), edgecolor=darker([0.5, 0.5, 0.5], alpha[i]),
                                                 linewidth=1))
    ...
    def draw_trajectory(self, summary=None, ep_split=False, episode='all', save_dir=None, robot_every_circle=True):
        ...
        #         all_reward += summary[m]['robot'][n]['reward']
        ...
        if episode != 'all' and episode != 'split' and int(episode) < ep_total:
            ...
            self._plot_t_ep(summary, episode, ax, xy_lim, obs_color, robot_every_circle)
            width_x = 3.487
            width_y = 3.487
