...
class Train:
    ...
    def Run(self)->None:
        ...
        for epoch in epoch_iter:
            ...
            if (self.rank == 0) and ((epoch+1) % 10000 == 0):
                self.model.save('./log/{}/'.format(self.name)+str(int((epoch+1)/10000))+'W.pt')
...
if __name__ == "__main__":
    ...
    dataloader = DL(dataset=dataset, batch_size=cfg['Dataset']['batch'], num_workers=os.cpu_count()//world_size, sampler=sampler, collate_fn=collate.collate_fn, drop_last=True, pin_memory=True)
    model = DDP(NaviDiffusion(**cfg['Network']).to(device), device_ids=[rank], output_device=rank)
