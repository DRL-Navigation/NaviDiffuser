...
class EnveDoubleMoActorAgent(object):
    def __init__(
        ...
        self.tb_writer = tensorboard.SummaryWriter(log_dir=tb_logdir)
    ...
    def get_action(self, state, preference):
        ...
        if self.training:
            ...
            action = torch.argmax(policy, dim=1).data.cpu().numpy()
    ...
    def train_model(
        ...
        # Total loss (don't compute tempreture)
        loss = actor_loss.mean()
        loss += 0.0001 * (self.beta * critic_loss_l1 + (1-self.beta) * critic_loss_l2)
        loss -= self.entropy_coef * entropy.mean()
        ...
        self.tb_writer.add_scalar('Actor Loss', actor_loss.mean().item(), global_step)
        self.tb_writer.add_scalar('Entropy', entropy.mean().item(), global_step)
        self.tb_writer.add_scalar('Critic_Loss_l1', critic_loss_l1.item(), global_step)
        self.tb_writer.add_scalar('Critic_Loss_l2', critic_loss_l2.item(), global_step)
